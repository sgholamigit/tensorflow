...
def runANN(X, hx, hy, test=False):
    ...
    if test:
        ...
        batch_size = 200
        encode_decode_data = np.zeros((X.shape[0], (image_size * image_size)))
        diff_sum = 0
        for i in range(X.shape[0] / batch_size):
            ...
            encode_decode_data[start:end] = reformat(np.asarray(model.predict(X[start:end])))

        X = reformat(X)
        for i in range(encode_decode_data.shape[0]):
            diff_sum = diff_sum + (sum(abs(X[i] - encode_decode_data[i])))/(image_size * image_size)
        print 'Average test error per pixel =' , diff_sum/X.shape[0]

        examples_to_show = 10
        f, a = plt.subplots(2, 10, figsize=(10, 2))
        for i in range(10):
            a[0][i].imshow(np.reshape(X[i], (image_size, image_size)), cmap='gray', interpolation='nearest')
            a[1][i].imshow(np.reshape(encode_decode_data[i], (image_size, image_size)), cmap='gray', interpolation='nearest')
        f.show()
        plt.draw()
        plt.waitforbuttonpress()
    else:
        model.fit(X, X, batch_size=256, n_epoch=100, validation_set=0.1, show_metric=False, run_id='auto_test')
